...
- **List `status` filter:** `toListOpts` always sends IN's record-state filter (`status=active|archived|deleted`, comma-separable), defaulting to `active` — IN's index endpoints otherwise return soft-deleted rows mixed in. The shared `status` input (`active|archived|deleted|all`) is record state only; entity-specific states go through other params mapped to IN filters (`client_status` on invoices *and* tasks — task invoiced/uninvoiced is `client_status`, not `status`).
